"""
Add the reviewed PureFyul V1 Tune rule for hemp milk.

Product guardrail:

- user-facing volume range: 70-260 mL
- explicit hemp-milk specific-gravity conversion: 1.0286 g/mL
- solver-facing gram range: 72.002-267.436 g
...
LOOKUP_NAME = "hemp milk"
MIN_WEIGHT_G = 72.002
MAX_WEIGHT_G = 267.436
...
def main() -> None:
    ...
    try:
        with conn.cursor() as cur:
            ...
            if row != expected:
                raise RuntimeError(
                    f"hemp milk Tune rule verification failed: {row!r}"
                )
    ...
    print(
        "SUCCESS: hemp milk Tune rule is ready "
        "(70-260 mL / 72.002-267.436 g)"
    )
